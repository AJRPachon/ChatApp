...
sealed interface SavedMessagesIntent {
    data object Refresh : SavedMessagesIntent
}
...
class SavedMessagesViewModel(
    ...
    fun onIntent(intent: SavedMessagesIntent) {
        when (intent) {
            is SavedMessagesIntent.Refresh -> loadMessages()
        }
    }
        ...
        viewModelScope.launch {
            ...
            messageRepository.getSavedMessages(currentUserId).collect { messages ->
                val items = messages.map { bo ->
                    val name = conversationRepository.getById(bo.conversationId)
                        ?.name?.takeIf { it.isNotBlank() } ?: "Conversacion"
                    SavedMessageItem(
                        id = bo.id,
