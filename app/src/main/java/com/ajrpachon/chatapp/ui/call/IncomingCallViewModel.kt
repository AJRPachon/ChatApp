...
class IncomingCallViewModel(
...
) : BaseViewModel<IncomingCallState, IncomingCallEffect>(IncomingCallState()) {
        ...
        viewModelScope.launch {
            catchResult {
                ...
                callRepository.observeIncomingCalls(user.id).collect { call ->
                    ...
                    updateState { IncomingCallState(incomingCall = call) }
    ...
    fun dismiss() {
        ...
        statusJob?.cancel()
        updateState { IncomingCallState(incomingCall = null) }
